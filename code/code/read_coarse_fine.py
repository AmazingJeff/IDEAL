...
def get_data(data_path, n_labeled_per_class, unlabeled_per_class_al=10000, unlabeled_per_class=5000, max_seq_len=66, model='bert-base-uncased', train_aug=False):
    ...
    train_df = pd.read_csv('~/workspace/' + data_path + 'train.csv', header=None)
    test_df = pd.read_csv('~/workspace/' + data_path + 'test.csv', header=None)
    ...
    train_labeled_idxs, train_unlabeled_al_idxs, train_unlabeled_idxs, val_idxs, initial_idxs = train_val_split(
        train_labels, n_labeled_per_class, unlabeled_per_class_al, unlabeled_per_class, n_labels)

    # Build the dataset class for each set
    train_labeled_dataset = loader_labeled(
        train_text[train_unlabeled_al_idxs], train_labels[train_unlabeled_al_idxs], tokenizer, max_seq_len, train_aug)

    coarse_fine = loader_unlabeled_aug(
        train_text[train_unlabeled_al_idxs], train_unlabeled_al_idxs, tokenizer, max_seq_len, Translator('/home/admin/workspace/' + data_path))

    train_unlabeled_dataset = loader_unlabeled(
        train_text[train_unlabeled_idxs], train_unlabeled_idxs, tokenizer, max_seq_len, Translator('/home/admin/workspace/' + data_path))
    ...
    initial_idxs = np.array([np.where(train_unlabeled_al_idxs == v)[0][0] for v in list(initial_idxs)])

    return train_labeled_dataset, train_unlabeled_dataset, coarse_fine, val_dataset, test_dataset, n_labels, initial_idxs


def train_val_split(labels, n_labeled_per_class, unlabeled_per_class_al, unlabeled_per_class, n_labels, seed=0):
    ...
    for i in range(n_labels):
        ...
        if n_labels == 2:
            ...
            initial_idxs.extend(train_pool[:n_labeled_per_class])
            train_unlabeled_al_idxs.extend(
                idxs[500: 500 + 5000])
            ...
            train_unlabeled_al_idxs.extend(
                idxs[500: 500 + unlabeled_per_class_al-n_labeled_per_class])
            val_idxs.extend(idxs[-2000:])
        else:

            initial_idxs.extend(idxs[:n_labeled_per_class])

            train_unlabeled_idxs.extend(
                idxs[500: 500 + 5000])

            train_unlabeled_al_idxs.extend(
                idxs[n_labeled_per_class:-2000])
            val_idxs.extend(idxs[-2000:])
    train_unlabeled_al_idxs.extend(initial_idxs)
    np.random.shuffle(train_labeled_idxs)
    np.random.shuffle(train_unlabeled_al_idxs)
    np.random.shuffle(train_unlabeled_idxs)
    np.random.shuffle(val_idxs)

    return train_labeled_idxs, train_unlabeled_al_idxs, train_unlabeled_idxs, val_idxs, initial_idxs
...
class loader_unlabeled_aug(Dataset):
    ...
    def __getitem__(self, idx):
        if self.aug is not None:
            ...
            text = self.text[idx]
            encode_result, length = self.get_tokenized(text)
            return (torch.tensor(encode_result), length, idx)